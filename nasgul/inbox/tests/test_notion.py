...
def test_upload_file_create_then_send(tmp_path):
    from inboxzero.notion import upload_file
    ...
    with patch("inboxzero.notion.httpx.post", side_effect=fake_post):
...
def test_add_item_posts_to_notion():
    ...
    with (
        patch("inboxzero.notion.httpx.post", side_effect=fake_post),
        patch("inboxzero.notion.httpx.get", side_effect=fake_get),
    ):
...
def test_discover_view_only_raises_instead_of_fallback():
    ...
    with patch("inboxzero.notion.httpx.get", side_effect=fake_get):
        try:
