...
function post(): RequestInit {
  return { method: "POST", credentials: "include", headers: { "X-Regie": "1" } };
}

function json(method: string, body: unknown): RequestInit {
  return { method, credentials: "include", headers: { "Content-Type": "application/json", "X-Regie": "1" }, body: JSON.stringify(body) };
}

export async function login(email: string, password: string): Promise<void> {
  await parse(await fetch("/api/v1/auth/login", json("POST", { email, password })));
}

export async function logout(): Promise<void> {
  await fetch("/api/v1/auth/logout", post());
}
...
export async function getQueue(
  ...
  const res = await fetch(`/api/v1/mail/queue${suffix}`, { credentials: "include", headers, signal });
...
export async function getItem(id: number): Promise<MailItem | null> {
  const data = (await parse(await fetch(`/api/v1/mail/items/${id}`, { credentials: "include" }))) as { item?: MailItem | null };
  return data.item ?? null;
}

export async function searchMails(q: string, signal?: AbortSignal): Promise<SearchPayload> {
  const params = new URLSearchParams({ q });
  return (await parse(await fetch(`/api/v1/mail/search?${params}`, { credentials: "include", signal }))) as SearchPayload;
}

export async function getMail(indexId: number): Promise<{ hit: SearchHit; body: string }> {
  return (await parse(await fetch(`/api/v1/mail/mails/${indexId}`, { credentials: "include" }))) as { hit: SearchHit; body: string };
}

export async function adoptMail(indexId: number, queue: boolean): Promise<MailItem | null> {
  const data = (await parse(await fetch(`/api/v1/mail/mails/${indexId}/adopt`, json("POST", { queue })))) as { item?: MailItem | null };
  return data.item ?? null;
}

export async function scan(): Promise<{ job_id?: number }> {
  return (await parse(await fetch("/api/v1/mail/scan", post()))) as { job_id?: number };
}

export async function performAction(kind: ActionKind, ids: number[], folder?: string): Promise<ActionResult> {
  return (await parse(await fetch("/api/v1/mail/actions", json("POST", { ids, kind, folder })))) as ActionResult;
}

export async function undoAction(actionId: number): Promise<ActionResult> {
  return (await parse(await fetch(`/api/v1/mail/actions/${actionId}/undo`, post()))) as ActionResult;
}

export async function listActions(limit = 40): Promise<ActionRow[]> {
  const data = (await parse(await fetch(`/api/v1/mail/actions?limit=${limit}`, { credentials: "include" }))) as { actions?: ActionRow[] };
  return data.actions ?? [];
}

export async function brief(id: number): Promise<{ job_id?: number; item?: MailItem | null }> {
  return (await parse(await fetch(`/api/v1/mail/items/${id}/brief`, post()))) as {
    job_id?: number;
    item?: MailItem | null;
  };
}
...
export async function addToNotion(id: number, note: string): Promise<{ job_id?: number; url?: string; item?: MailItem | null }> {
  return (await parse(await fetch(`/api/v1/mail/items/${id}/notion`, json("POST", { note })))) as {
    job_id?: number;
    url?: string;
    item?: MailItem | null;
  };
...
export async function getJob(id: number): Promise<Job | null> {
  const data = (await parse(await fetch(`/api/v1/mail/v1/jobs/${id}`, { credentials: "include" }))) as { job?: Job | null };
  return data.job ?? null;
}

export async function openAttachment(itemId: number, n: number): Promise<Blob> {
  const res = await fetch(`/api/v1/mail/items/${itemId}/attachments/${n}`, { credentials: "include" });
  if (res.status === 401) throw new AuthError();
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    const err =
...
export async function getSettings(probe = false): Promise<SettingsPayload> {
  return (await parse(await fetch(`/api/v1/mail/settings${probe ? "?probe=1" : ""}`, { credentials: "include" }))) as SettingsPayload;
}
...
export async function saveSettings(body: {
  ...
  await parse(await fetch("/api/v1/mail/settings", json("POST", body)));
}

export const EVENTS_URL = "/api/v1/stream";
