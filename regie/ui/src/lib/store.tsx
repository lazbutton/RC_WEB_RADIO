    ...
    function connect() {
      ...
      source.addEventListener("job", (ev) => {
        try {
          ...
          job.kind = job.kind.replace(/^mail\./, "");
          setJobs((prev) => {
      ...
      for (const type of ["notification", "task.created", "task.updated", "task.deleted", "tasks", "calendar", "events", "episode", "podcast", "publish", "publications", "coverage.planned", "coverage.updated", "guest.created", "guest.status", "booking.created", "person.created", "person.updated", "organization.created", "link.created", "link.removed", "job", "action", "queue"]) {
        source.addEventListener(type, (ev) => {
          ...
          window.dispatchEvent(new CustomEvent("regie-event", { detail: { type, data } }));
        });
      }
      source.addEventListener("action", (ev) => {
        try {
          const data = JSON.parse((ev as MessageEvent).data) as { action?: ActionRow & { module?: string; item_ids?: Array<number | string> }; status?: string };
          if (data.action && (!data.action.module || data.action.module === "mail")) {
            const row: ActionRow = { ...data.action, kind: data.action.kind.replace(/^mail\./, "") as ActionRow["kind"], item_ids: (data.action.item_ids || []).map((i) => Number(i)), created_rel: data.action.created_rel || "" };
            setActions((prev) => {
