...
- Alertes (`alerts:` dans `radiotomate.yaml`) : silence, playout injoignable, heartbeat perdu, créneau live sans encodeur (EF-01), sortie Icecast déconnectée (`icecast_down`, locale ou relais public) → log, webhook, tâche Vikunja. Les incidents ouverts sont persistés dans `/data/alerts.json` : après un redémarrage du scheduler, le rétablissement clôt la bonne tâche.
- `GET /antenne/state.json` (scheduler, relayé par l'interface) : état agrégé pour la page Antenne de la console — santé Liquidsoap, sorties Icecast et auditeurs (`status-json.xsl`), incidents, 8 derniers passages, prochain créneau live. `POST /antenne/flush.json` vide les files.
- Conteneurs `interface` et `scheduler` en `network_mode: service:radiotomate-playout` : **ne jamais redémarrer `radiotomate-playout` seul** (ils perdent le réseau) ; recréer les trois (`compose up -d --force-recreate radiotomate-playout radiotomate-interface radiotomate-scheduler`), ce que fait `nasgul-update.sh` quand l'image change.
